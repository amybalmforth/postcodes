...
- Anything within the Lambeth and Southwark LSOAs
...
* Postcode class - Stores the input postcode, makes the API call with it, stores the response body

* Result class - Accesses the API response and extracts the LSOA details, assigns to Whitelist class

* Whitelist class - Confirms if the LSOA is servable, non servable or not a valid postcode

I built a separate Whitelist class in case the user wishes to change the whitelist later on - at the moment it is set up to include Southwark and Lambeth but this can be changed out later.
...
2.2.10 :001 > require './models/postcode.rb'
 => true
2.2.10 :002 > result = Result.new(postcode = Postcode.new('SW81HF'), whitelist = Whitelist.new)
...
2.2.10 :001 > require './models/postcode.rb'
 => true
2.2.10 :002 > result = Result.new(postcode = Postcode.new('none'), whitelist = Whitelist.new)
